...
cairo_surface_t *
createCairoEglSurface(struct window *window, struct ESContext *_escontext,
...
- (id)initWithDevice:(void *)device
{
  ...
  // the egl surface is going to be created when we need to display the surface
  // on screen
  _eglSurface = NULL;
...
- (void)handleExposeRect:(NSRect)rect
{
  ...
  if (window->configured)
      ...
      if (_eglSurface == NULL)
	{
	  ...
	  _eglSurface = createCairoEglSurface(window, &(self->_escontext),
					      self->_cairo_device);
	  eglcr = cairo_create(_eglSurface);
	}
      // make the context current
      makeCurrent(&(_escontext));

      NSDebugLog(@"[%d] blit", window->window_id);
      // blit the mem surface to egl surface
      cairo_surface_set_device_offset(_memSurface, 0, 0);

      cairo_rectangle(eglcr, 0, 0, window->width, window->height);
      cairo_clip(eglcr);
      cairo_set_source_surface(eglcr, _memSurface, 0, 0);
      cairo_paint_with_alpha(eglcr, 1.0);
      int err;
      if (err = cairo_status(eglcr) != CAIRO_STATUS_SUCCESS)
	{
    ...
    {
      NSDebugLog(@"[%d] handleExposeRect skip", window->window_id);
      if (_eglSurface)
	{
...
{
  struct window *window = (struct window *) gsDevice;
  NSDebugLog(@"[%d] -----> destroy eglSurface", window->window_id);
